...
def scraper(grid_size, images_per_grid, image_shape, keep_current_images=True, bounding_box=None, location_name=None, timeout_minutes=30):
    ...
    max_images = images_per_grid * grid_size * grid_size * 8
    ...
    if not keep_current_images:
        counter = 0
        metadata_json = {'counter': 0, 'metadata': [], 'grid_image_counter': {i: {j: 0 for j in range(grid_size)} for i in range(grid_size)}}
        num_classes = 0
